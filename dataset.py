...
class CAS_Dev_Dataset(object):
    ...
    def get_file_list(self):
        ...
        selected_data = self.stats_csv[self.stats_csv['scene_label'].isin(self.selected_scene_list)]
    ...
    def get_tensordataset(self):
        ...
        if self.is_train:
            loader = DataLoader(dataset=dataset, batch_size=self.batch_size, shuffle=True, num_workers=4, pin_memory=True)
        else:
            loader = DataLoader(dataset=dataset, batch_size=self.batch_size, shuffle=False, num_workers=4, pin_memory=True)
        return loader
